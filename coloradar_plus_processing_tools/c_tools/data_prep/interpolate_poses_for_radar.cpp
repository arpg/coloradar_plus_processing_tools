...
int main(int argc, char** argv) {
    ...
    fs::path outputFilePath(outputFile.empty() ? "radar_poses_interpolated.txt" : outputFile);

    coloradar::ColoradarDataset dataset(coloradarDir);
    auto run = dataset.getRun(runName);
    auto poses = run->getPoses<octomath::Pose6D>();
    auto posesInterpolated = run->interpolatePoses(poses, run->poseTimestamps(), run->cascadeTimestamps());
